...
<div class="py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
    ...
    <nav class="mb-4 flex items-center space-x-2 text-sm text-gray-500">
        <a href="{{ route('hostels.index') }}" class="hover:text-gray-700">Hostels</a>
        <i class="bi bi-chevron-right text-xs"></i>
        <span class="text-gray-900">Edit Hostel</span>
    </nav>
        ...
        <div class="p-6 border-b border-gray-200">
            <div class="flex items-center gap-3">
                <div class="bg-blue-100 p-2 rounded-lg border border-blue-200">
                    <i class="bi bi-house-door text-blue-600 text-xl"></i>
                </div>
                <h1 class="text-2xl font-bold text-gray-900">Edit Hostel</h1>
            </div>
        </div>

        <div class="p-6">
            <form action="{{ route('hostels.update', $hostel) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4"
                                class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('description', $hostel->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600 flex items-center gap-1"><i class="bi bi-exclamation-circle"></i>{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="available_rooms" id="available_rooms" min="0"
                                value="{{ old('available_rooms', $hostel->available_rooms) }}"
                                class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                                required>
